...
export const bridge = {
  ...
  project: {
    ...
    renameRecent: (projectPath, displayName) => getBridge()?.project.renameRecent(projectPath, displayName),
  },
  environment: {
  ...
  chat: {
    ...
    models: (forceRefresh) => getBridge()?.chat?.models?.(forceRefresh),
    agents: () => getBridge()?.chat?.agents?.(),
  ...
  git: {
    ...
    statusModel: () => getBridge()?.git.statusModel?.(),
    refresh: () => getBridge()?.git.refresh?.(),
    diff: (path) => getBridge()?.git.diff(path),
    diffCached: (path) => getBridge()?.git.diffCached?.(path),
  },
  events: {
